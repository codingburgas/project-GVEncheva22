...
const DECK_ID = "taskflow-presentation";
const OUT_DIR = "/Users/gabriela/Documents/2526-11V-SD-GVEncheva22/tmp/slides/taskflow-presentation/output";
const REF_DIR = "/Users/gabriela/Documents/2526-11V-SD-GVEncheva22/tmp/slides/taskflow-presentation/reference";
...
const INK = "#122033";
const GRAPHITE = "#2F4057";
const MUTED = "#6A7891";
const PAPER = "#F4F7FB";
const PAPER_96 = "#F4F7FBF2";
const WHITE = "#FFFFFF";
const ACCENT = "#25C2C7";
const ACCENT_DARK = "#0F3D56";
const GOLD = "#FFB84D";
const CORAL = "#FF7A59";
...
const SOURCES = {
  primary: "Project assignment requirements from проект 17.docx and implemented TaskFlow MVC project features.",
};

const SLIDES = [
  {
    kicker: "TASK MANAGER PROJECT",
    title: "TaskFlow",
    subtitle: "A web-based Kanban-inspired project and task management system built with ASP.NET Core MVC.",
    moment: "Organize boards, tasks, priorities, and deadlines in one place.",
    notes: "Introduce the product name, the purpose of the system, and the overall direction of the project.",
    sources: ["primary"],
  },
  {
    kicker: "PROJECT OVERVIEW",
    title: "What is TaskFlow?",
    subtitle: "TaskFlow is designed to help users plan work visually through boards and tasks while tracking progress in a clear and structured way.",
    cards: [
      ["Main Goal", "Develop a web application for project and task management inspired by Trello and the Kanban methodology."],
      ["Core Idea", "Users can create boards, add tasks, assign priorities, define deadlines, and monitor status from To Do to Done."],
      ["User Value", "The system improves organization, visibility, and productivity through simple navigation and practical task tracking."],
    ],
    notes: "Explain the purpose of the app and the problem it solves for users.",
    sources: ["primary"],
  },
  {
    kicker: "FUNCTIONAL FEATURES",
    title: "Main Functionalities",
    subtitle: "The application covers the key requirements of the assignment through task organization, board management, and reporting views.",
    metrics: [
      ["CRUD", "Boards and Tasks", "Create, read, update, and delete actions are implemented for the main entities."],
      ["3", "Task Statuses", "Tasks can move through To Do, In Progress, and Done states."],
      ["7 DAYS", "Deadline View", "Dashboard cards and upcoming deadline tracking help users identify urgent work."],
    ],
    notes: "Highlight the practical features users see and interact with every day.",
    sources: ["primary"],
  },
  {
    kicker: "ARCHITECTURE",
    title: "Software Structure and Data Layer",
    subtitle: "The project follows layered MVC principles and demonstrates object-oriented design together with persistent database storage.",
    cards: [
      ["MVC Pattern", "Controllers, models, services, and Razor views are separated to keep responsibilities clear and maintainable."],
      ["EF Core + SQLite", "The database is created through Code-First models and stores users, boards, and tasks permanently."],
      ["OOP Principles", "BaseEntity, interfaces, inheritance, encapsulation, and service abstraction are used throughout the project."],
    ],
    notes: "Describe how the codebase is organized and why the architecture is suitable for this type of app.",
    sources: ["primary"],
  },
  {
    kicker: "SECURITY AND FLOW",
    title: "Security, Validation, and User Experience",
    subtitle: "TaskFlow combines authentication, validation, and a responsive interface to create a secure and usable application.",
    cards: [
      ["Authentication", "Users can register and log in with ASP.NET Identity, while role handling supports User and Admin access levels."],
      ["Validation", "Data Annotations such as Required and StringLength protect the correctness of titles, descriptions, and form inputs."],
      ["UI Design", "Razor views and Bootstrap provide a clean, modern interface for boards, task cards, forms, and dashboard pages."],
    ],
    notes: "Explain that the application is not only functional, but also validated and secure.",
    sources: ["primary"],
  },
  {
    kicker: "FINAL SUMMARY",
    title: "TaskFlow in One View",
    subtitle: "The project successfully combines task management, database persistence, MVC architecture, and role-based security in a complete school software product.",
    metrics: [
      ["MVC", "Web Architecture", "Clear separation of models, views, and controllers."],
      ["LINQ", "Data Logic", "Filtering by priority and grouping by status are built into the system."],
      ["READY", "Project Outcome", "TaskFlow delivers the required functionality in English with a polished interface."],
    ],
    notes: "Close the presentation by emphasizing the final result and readiness of the project.",
    sources: ["primary"],
  },
];
...
function addReferenceCaption(slide, slideNo) {
  addText(
    slide,
    slideNo,
    "TaskFlow MVC | Project presentation summary",
    64,
    674,
    980,
    22,
...
async function slideCover(presentation) {
  ...
  addShape(slide, "rect", 0, 0, W, H, "#EEF4FB", TRANSPARENT, 0, { slideNo, role: "cover contrast overlay" });
  addShape(slide, "roundRect", 820, 0, 460, 720, "#16324F", TRANSPARENT, 0, { slideNo, role: "cover side panel" });
  addShape(slide, "ellipse", 910, 110, 220, 220, "#25C2C733", TRANSPARENT, 0, { slideNo, role: "cover decor circle" });
  addShape(slide, "ellipse", 1050, 210, 120, 120, "#FFB84D55", TRANSPARENT, 0, { slideNo, role: "cover decor circle" });
  addShape(slide, "roundRect", 884, 360, 250, 120, "#FFFFFF18", TRANSPARENT, 0, { slideNo, role: "cover decor card" });
  addShape(slide, "roundRect", 930, 510, 180, 84, "#25C2C7", TRANSPARENT, 0, { slideNo, role: "cover decor card" });
  ...
  addText(slide, slideNo, "Boards\nTasks\nDeadlines\nDashboard", 900, 376, 180, 140, {
    size: 24,
    color: WHITE,
    bold: true,
    face: TITLE_FACE,
    role: "cover side keywords",
  });
...
async function slideCards(presentation, idx) {
  ...
  addShape(slide, "rect", 0, 0, W, H, "#F4F7FB", TRANSPARENT, 0, { slideNo: idx, role: "content contrast overlay" });
  addShape(slide, "roundRect", 990, 84, 210, 560, "#16324F", TRANSPARENT, 0, { slideNo: idx, role: "side rail" });
  addShape(slide, "ellipse", 1040, 120, 110, 110, "#25C2C744", TRANSPARENT, 0, { slideNo: idx, role: "side decor" });
  addShape(slide, "ellipse", 1080, 518, 72, 72, "#FF7A5955", TRANSPARENT, 0, { slideNo: idx, role: "side decor" });
  ...
  for (let cardIdx = 0; cardIdx < cols; cardIdx += 1) {
    ...
    addCard(slide, idx, x, 404, cardW, 198, label, body, { iconKind: iconKinds[cardIdx % iconKinds.length] });
  }
  addText(slide, idx, "Clean workflow\nRole-based access\nFast navigation", 1022, 268, 148, 126, {
    size: 18,
    color: WHITE,
    bold: true,
    face: TITLE_FACE,
...
async function slideMetrics(presentation, idx) {
  ...
  const slide = presentation.slides.add();
  await addPlate(slide, idx);
  addShape(slide, "rect", 0, 0, W, H, "#EEF4FB", TRANSPARENT, 0, { slideNo: idx, role: "metrics contrast overlay" });
  addShape(slide, "roundRect", 62, 250, 1156, 358, "#FFFFFF", "#D9E2EF", 1, { slideNo: idx, role: "metrics stage" });
  addHeader(slide, idx, data.kicker, idx, SLIDES.length);
  ...
  addShape(slide, "ellipse", 1035, 130, 92, 92, "#25C2C733", TRANSPARENT, 0, { slideNo: idx, role: "metrics decor" });
  addShape(slide, "ellipse", 1110, 190, 48, 48, "#FFB84D66", TRANSPARENT, 0, { slideNo: idx, role: "metrics decor" });
  addReferenceCaption(slide, idx);
  addNotes(slide, data.notes, data.sources);
}
